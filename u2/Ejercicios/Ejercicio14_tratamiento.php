...
<body>
    <?php
    echo '<h1>Tratamiento de formularios ejercicio 14</h1>';
    //Chequear si se llama por POST
    if (isset($_POST['validar'])) {
        //Chequear que todos los campos se han rellenado
        if (
            empty($_POST['nombre']) or empty($_POST['apellidos']) or empty($_POST['ps']) or
            empty($_POST['fechaN']) or isset($_FILES['foto']) or empty($_POST['comentario']) or
            !isset($_POST['afic'])
        ) {
        ...
        //Fecha de nacimiento con el formato HTML dd-mm-yyyy
        ...
        echo '<div>Archivo:' . $_FILES['foto']['name'] . '</div>';
        echo '<div>Aficiones:';
        foreach ($_POST['afic'] as $a) {
            echo $a . ' ';
        }
        echo '</div>';
